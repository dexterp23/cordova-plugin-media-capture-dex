...
import android.widget.Toast; //custom
import android.media.MediaMetadataRetriever; //custom
import java.io.FileInputStream; //custom
import java.io.FileNotFoundException; //custom
...
//    private static final int CAPTURE_INVALID_ARGUMENT = 2;
	...
	private int countdownchk; //custom
	long deley_chk = 0;
	private Toast mytoast; //custom
	public static final int MY_PERMISSIONS_REQUEST_READ_EXTERNAL_STORAGE = 1; //custom
...
//    }
    ...
    private void captureVideo(Request req) {
        ...
        if (needExternalStoragePermission || needCameraPermission || needMicrophonePermission) {
            if (needExternalStoragePermission && needCameraPermission && needMicrophonePermission) {
                PermissionHelper.requestPermissions(this, req.requestCode, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.CAMERA, Manifest.permission.RECORD_AUDIO});
            } else if (needExternalStoragePermission && needCameraPermission) {
                PermissionHelper.requestPermissions(this, req.requestCode, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.CAMERA});
            } else if (needExternalStoragePermission && needMicrophonePermission) {
                PermissionHelper.requestPermissions(this, req.requestCode, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.RECORD_AUDIO});
            } else if (needCameraPermission && needMicrophonePermission) {
                PermissionHelper.requestPermissions(this, req.requestCode, new String[]{Manifest.permission.CAMERA, Manifest.permission.RECORD_AUDIO});
            } else if (needExternalStoragePermission) {
                PermissionHelper.requestPermission(this, req.requestCode, Manifest.permission.READ_EXTERNAL_STORAGE);
            } else if (needCameraPermission) {
                PermissionHelper.requestPermission(this, req.requestCode, Manifest.permission.CAMERA);
            } else if (needMicrophonePermission) {
                PermissionHelper.requestPermission(this, req.requestCode, Manifest.permission.RECORD_AUDIO);
            }
        } else {
			
			//custom
			if (req.countdownchk == 1) {
				
				//mytoast = Toast.makeText(cordova.getActivity(), String.valueOf(req.countdownchk), Toast.LENGTH_LONG);
				//mytoast.show();
    ...
    public void onVideoActivityResult(Request req, Intent intent) {
        ...
        if (intent != null){
			...
			deley_chk = intent.getLongExtra("deley_chk", 0); //custom
			//Log.i(TAG, String.valueOf("deley_chk intent: " + intent.getLongExtra("deley_chk", 0))); //custom
        }
    ...
    private JSONObject createMediaFile(Uri data) {
		...
		if (countdownchk == 1) {
        	...
        	fp = webView.getResourceApi().mapUriToFile(data);
		}
        ...
        try {
            ...
            // File properties
			obj.put("duration", video_duration); //custom (treba dodati i u fajlu "www/capture.js")
			obj.put("deley_chk", deley_chk); //custom
